...
// import { addWhatsAppUser, findUserBySession } from '../utils/whatsapp-storage.js'; // Temp disabled
// import { logConversation, detectIntent } from '../utils/sheets-logger.js'; // Temp disabled
// import { getSessionData, setSessionData, clearSessionData } from '../utils/session-store.js'; // Temp disabled
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const reply = `Ciao! 👋 Benvenuto alle Lucine di Natale!\n\nHai scritto: "${message}"\n\nSiamo operativi dal 5 dicembre 2024 al 6 gennaio 2025, dalle 17:30 alle 22:00.\n\nCome posso aiutarti?`;
    ...
    // 🔴 CHECK: Se sessione è in chat live con operatore (temp disabled)
    const sessionData = {}; // Temporarily use empty object
    // const sessionData = getSessionData(sessionId);
    
    // Check if session is taken by operator (simplified)
    if (sessionData.mode === 'live_chat_active' || sessionData.waiting_for_operator) {
      // Update session mode if operator took the chat (temp disabled)
      // if (sessionData.mode !== 'live_chat_active') {
      //   setSessionData(sessionId, { mode: 'live_chat_active' });
      // }
      
      // Chat è gestita da operatore umano
      console.log(`💬 Utente in chat live: ${message}`);
      
      // Controlla se ci sono messaggi dall'operatore nel session data
      const operatorMessages = sessionData.operator_messages || [];
    ...
    // Temporarily disable OpenAI to test basic functionality
    console.log('🤖 Processing message:', message);
    
    let reply = `Ciao! 👋 Benvenuto alle Lucine di Natale di Leggiuno!\n\nGrazie per il tuo messaggio: "${message}"\n\n🎄 Siamo aperti dal 5 dicembre 2024 al 6 gennaio 2025\n⏰ Orario: 17:30 - 22:00\n🎫 Biglietti disponibili online\n\nCome posso aiutarti?`;
    ...
    if (reply.includes('WHATSAPP_REQUEST') || message.toLowerCase().includes('whatsapp') || 
      ...
      if (phoneMatch) {
        ...
        // Salva nel database (temporarily disabled)
        const currentSessionId = sessionId || generateSessionId();
        console.log(`📱 WhatsApp number collected: ${formattedPhone} for session ${currentSessionId}`);
        // const saved = addWhatsAppUser(currentSessionId, formattedPhone, {
        //   source: 'chatbot',
        //   user_agent: req.headers['user-agent'],
        //   ip: req.headers['x-forwarded-for'] || req.connection.remoteAddress
        // });
        const saved = true; // Temporarily always true
        
        if (saved) {
          // Invia notifica di benvenuto WhatsApp (temporarily disabled)
          console.log(`📱 WhatsApp welcome message would be sent to: ${formattedPhone}`);
          // try {
    ...
    const intentDetected = 'chat_ai'; // Simplified for now
    const whatsappUser = null; // Simplified for now
    
    // Salva conversazione (temporarily disabled)
    console.log('📊 Conversation log (disabled):', { sessionId: sessionId || generateSessionId(), message: message.substring(0, 50) });
    // logConversation({
    //   sessionId: sessionId || generateSessionId(),
    //   userMessage: message,
    //   botReply: reply,
...
// Verifica disponibilità operatori (simplified)
async function checkOperatorAvailability() {
  try {
    // Per ora ritorna sempre false - l'operatore può prendere manualmente
    // La dashboard funziona perfettamente per vedere pending sessions
    console.log('👨‍💼 Check operatori: fallback a ticket per ora');
    return false; // Sempre ticket per ora - operatori lavorano da dashboard
  } catch (error) {
...
async function initiateLiveChat(sessionId, originalQuestion, res) {
  try {
    ...
    // Imposta stato sessione per handover (temp disabled)
    // setSessionData(sessionId, {
...
// Sistema semplificato - la dashboard operatori funziona perfettamente
// Gli operatori usano la dashboard per gestire le chat
// Questo mantiene chat.js funzionante per le richieste normali
