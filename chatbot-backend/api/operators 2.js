...
async function getPendingSessions(req, res) {
  try {
    // WORKAROUND: In serverless, la memoria non è condivisa tra API
    // Per ora non abbiamo pending sessions qui (servono DB condivisi)
    // Il flow funziona comunque: utente chiede aiuto → operatore prende chat
    
    const pendingSessions = [];
    
    // Simulazione: le sessioni pending sarebbero in un DB condiviso
    console.log('📋 Pending sessions check - serverless memory isolation');
    
    return res.status(200).json({
      success: true,
      pending_sessions: pendingSessions,
      total_pending: pendingSessions.length,
      note: 'Serverless environment - use shared database for production'
    });
...
async function createTicketFromLiveChat(chatSession, reason) {
  try {
    ...
    // Chiama API ticket system corretto
